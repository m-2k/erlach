...
% 	#dtl{file="erlach",app=review,bindings=[{body,Content}, {theme,Theme}, {title,Title}]}.
...
header() -> header(u:get()).
header(User) ->
	% {ok, Svg} = file:read_file("apps/review/priv/static/svg/erlach-logo-2.svg"),
	Svg = logo(),
    [
...
board_body(#board{id=Bid,name=Name,description=Description}=Board, Content) ->
    ...
    Manage = #panel{class= <<"center">>,body=[
		#link{class = <<"button primary slim">>, body = <<"New">>, url = wf:f("/thread?board=~p",[Bid]) },
		case u:is_admin() of
			true ->
