...
convert_async(Pid, {Mime, Ext}, Binary, Quality, Meta) when is_pid(Pid)->
	?MODULE:start_link(),
	gen_server:cast(?MODULE, {convert, {Pid, {Mime, Ext}, Binary, Quality, Meta}}).

% gen_server:cast(image, calcbad).
% supervisor:start_child(web_sup, {image, {image, start_link, []}, temporary,10,worker,[]}).

start_link() ->
	wf:info(?MODULE, "Starting Gen Server from ~p...",[self()]),
    gen_server:start_link({local, ?MODULE}, ?MODULE, [], []).

init(_Args) ->
     % erlang:process_flag(trap_exit, true),
	 wf:info(?MODULE, "Gen Server started at ~p", [self()]),
...
handle_cast({convert, {Pid, {Mime, Ext}, Binary, Quality, Meta}}, State) ->
     io:format("handle_cast :: convert~n"),
     Response = convert({Mime, Ext}, Binary, Quality),
	 
	 % R = create_answer(Response, Meta),
	 % wf:info(?MODULE, "Response ~p ~p ~p", [Pid,self(),R]),
	 Pid ! {server, {converted, Response, Meta}},
     {noreply, State}.

handle_info(_Message, State) ->
	% io:format("image: unknown message.~n"),
	wf:info(?MODULE, "Unknown message ~p", [_Message]),
	{noreply, State}.

terminate(_Reason, _State) ->
     wf:info("Terminating Gen Server.~n"),
     ok.
...
convert({Mime, Ext}, Binary, Quality) ->
	case {Mime, Ext} of
		% ?MIME_IMAGE_JPEG ->
		% 	JpegQuality = 82,
		% 	{ok,GD} = gd:new(),	% {ok,{gd,#Port<0.10123>}}
		% 	{ok, Ptr} = gd:image_create_from_jpeg_ptr(GD, Binary),	% {ok,0}
		% 	{ok, Converted} = gd:image_jpeg_ptr(GD, Ptr, JpegQuality),
		% 	Converted;
		?MIME_IMAGE_JPEG ->
			try
				image:optim({Mime, Ext}, Binary, Quality)
				% image:to_bpg({Mime, Ext}, Binary, Quality)
		...
		?MIME_IMAGE_GIF ->
			try
				...
				{ok, ?MIME_IMAGE_GIF, Converted}
			catch E:R -> {error, {E,R}} end
	end.
	% wf:info(?MODULE, "Image converting (~p) OK. Original: ~p, Converted: ~p", [Mime2, size(Binary), size(Binary2)]),
	% {ok, Mime2, Binary2}.
...
optim({_, Ext}=Mime, Image, Quality) ->
	...
	case emagick:convert(Image, wf:to_atom(Ext), wf:to_atom(Ext), Opts, ?EMAGICK_APP_ENV) of
		{ok, [Result]} -> wf:info(?MODULE, "Image converting OK", []), {ok, Mime, Result};
		Err -> wf:info(?MODULE, "Image converting error: ~p", [Err]), Err
	end.
